...
// Square ghost icon-button: fixed 34x34 so the icon reads clearly in table rows.
const rowActionBtn =
  'ui-btn ui-btn--ghost has-tt relative z-[1] inline-flex items-center justify-center ' +
  'h-[34px] w-[34px] p-0 rounded-md border border-transparent bg-transparent text-text ' +
  'whitespace-nowrap select-none cursor-pointer ' +
  'transition duration-200 ease-[ease] motion-reduce:transition-none ' +
  'hover:bg-primary-soft hover:text-primary active:bg-grey-200 active:scale-[0.96] ' +
...
// 20px icons render crisply at 34px button size
const ICON = 20;
...
export default function RowActions({ onView, onEdit, onDownload, onDelete, onUpload }) {
  return (
    <div className="relative z-[1] flex items-center gap-1.5">
      {onView && (
        <button type="button" aria-label="View" className={rowActionBtn} onClick={onView}>
      ...
      {onEdit && (
        <button type="button" aria-label="Edit" className={rowActionBtn} onClick={onEdit}>
          <svg width={ICON} height={ICON} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      {onDownload && (
        <button type="button" aria-label="Download" className={rowActionBtn} onClick={onDownload}>
          <svg width={ICON} height={ICON} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      {onDelete && (
        <button type="button" aria-label="Delete" className={rowActionBtn} onClick={onDelete}>
          <svg width={ICON} height={ICON} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      {onUpload && (
        <button type="button" aria-label="Upload Image" className={rowActionBtn} onClick={onUpload}>
          <svg width={ICON} height={ICON} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
