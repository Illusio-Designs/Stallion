  ...
  return (
    ...
    >
      <div className="breadcrumb-container flex w-full flex-col items-start gap-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
        <ol className="breadcrumb-content m-0 flex min-w-0 list-none flex-wrap items-center gap-0.5 p-0 sm:gap-1 leading-[var(--leading-snug)] text-text-muted text-[length:var(--text-sm)] md:text-[length:var(--text-base)] lg:text-[length:var(--text-md)]">
          {breadcrumbItems.map((item, index) => {
            ...
            return (
              <li className="breadcrumb-crumb inline-flex min-w-0 items-center" key={item.id}>
                {index > 0 && (
                  <span className="breadcrumb-separator inline-flex shrink-0 items-center text-text-subtle" aria-hidden="true">
                ...
                {isCurrent ? (
                  <span className="breadcrumb-item active m-0 inline-block max-w-none cursor-default whitespace-normal rounded-sm px-2 py-1 font-semibold text-text-subtle" aria-current="page" title={item.text}>
                    {item.text}
                  </span>
                ) : (
                  <button
                    type="button"
                    className="breadcrumb-item m-0 inline-block max-w-[110px] cursor-pointer overflow-hidden text-ellipsis whitespace-nowrap rounded-sm border-none bg-transparent px-1 py-1 text-text transition-colors duration-200 ease-[ease] hover:bg-grey-100 hover:text-text active:bg-grey-200 focus-visible:shadow-[0_0_0_2px_rgba(0,0,0,0.3)] focus-visible:outline-none motion-reduce:transition-none sm:max-w-[150px] md:max-w-[220px] md:px-2"
