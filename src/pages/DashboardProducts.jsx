  ...
  const rows = useMemo(() => {
    return products.map(product => {
      ...
      return {
        ...
        gender_name: product.gender?.gender_name || product.gender_name || gender?.gender_name || gender?.name || 'N/A',
        color_code_id: product.color_code_id || '',
        color_code_name: product.color_code?.color_code || product.color_code_name || colorCode?.color_code || colorCode?.name || 'N/A',
        shape_id: product.shape_id || '',
        shape_name: product.shape?.shape_name || product.shape_name || shape?.shape_name || shape?.name || 'N/A',
        lens_color_id: product.lens_color_id || '',
        lens_color_name: product.lens_color?.lens_color || product.lens_color_name || lensColor?.lens_color || lensColor?.name || 'N/A',
        frame_color_id: product.frame_color_id || '',
        frame_color_name: product.frame_color?.frame_color || product.frame_color_name || frameColor?.frame_color || frameColor?.name || 'N/A',
        frame_type_id: product.frame_type_id || '',
        frame_type_name: product.frame_type?.frame_type || product.frame_type_name || frameType?.frame_type || frameType?.name || 'N/A',
        lens_material_id: product.lens_material_id || '',
        lens_material_name: product.lens_material?.lens_material || product.lens_material_name || lensMaterial?.lens_material || lensMaterial?.name || 'N/A',
        frame_material_id: product.frame_material_id || '',
        frame_material_name: product.frame_material?.frame_material || product.frame_material_name || frameMaterial?.frame_material || frameMaterial?.name || 'N/A',
        brand_id: product.brand_id || '',
        brand: product.brand?.brand_name || product.brand_name || brand?.brand_name || 'N/A',
        collection_id: product.collection_id || '',
        collection: product.collection?.collection_name || product.collection_name || collection?.collection_name || 'N/A',
      ...
      >
        <form className="ui-form" onSubmit={handleSubmit}>
          ...
          <div className="form-group">
            ...
            />
            <p className="m-0 mt-1 text-[length:var(--text-xs)] text-text-muted">
              Auto-calculated: Warehouse + Tray ({parseInt(formData.tray_qty) || 0} in trays).
            </p>
          </div>
          <div className="form-group form-group--full">
            <label className="ui-label">Status *</label>
